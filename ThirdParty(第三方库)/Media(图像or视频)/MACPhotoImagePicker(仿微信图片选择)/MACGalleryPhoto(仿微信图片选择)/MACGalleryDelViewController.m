...
@interface MACGalleryDelViewController (Private)
...
@implementation MACGalleryDelViewController{
...
@synthesize assetArray,selectedArray,isPreview,limitNum;
...
//    numLabel.hidden = YES;
   ...
   // self.navigationController.navigationBar.translucent = NO;
    ...
    checkBt = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 25, 25)];
    [checkBt setBackgroundImage:[UIImage imageNamed:@"deletePic"] forState:UIControlStateNormal];
    [checkBt setBackgroundImage:[UIImage imageNamed:@"deletePic"] forState:UIControlStateSelected];
    [checkBt setBackgroundImage:[UIImage imageNamed:@"deletePic"] forState:UIControlStateHighlighted];
...
-(void)selectBtTaped:(UIButton*)bt{
    if (_currentIndex>=assetArray.count) {
        [self.navigationController popViewControllerAnimated:YES];
        return;
    }
    [assetArray removeObjectAtIndex:_currentIndex];
    if (assetArray.count<=0) {
        [self.navigationController popViewControllerAnimated:YES];
        return;
    }
...
- (void)next
{
    NSUInteger numberOfPhotos = assetArray.count;
...
{
    NSUInteger numPhotos =assetArray.count;
...
- (void)updateScrollSize
{
    float contentWidth = _scroller.frame.size.width *  assetArray.count;
...
- (void)updateTitle
{
    ...
    if (!_hideTitle){
        [self setTitle:[NSString stringWithFormat:@"%li %@ %lu", _currentIndex+1, NSLocalizedString(@"/", @"") , (unsigned long)assetArray.count]];
    }else{
        [self setTitle:@""];
    }
   // [self refreshCheckBt];
...
- (void)buildThumbsViewPhotos
{
    NSUInteger i, count = assetArray.count;//[_photoSource numberOfPhotosForPhotoGallery:self];
    for (i = 0; i < count; i++) {
...
- (void)arrangeThumbs
{
    ...
    [_thumbsView setContentSize:CGSizeMake( _thumbsView.frame.size.width - ( kThumbnailSpacing*2 ), 0)];
}
...
- (void)loadAllThumbViewPhotos
{
    NSUInteger i, count = assetArray.count;
    for (i=0; i < count; i++) {
...
- (MACGalleryPhoto*)createGalleryPhotoForIndex:(NSUInteger)index
{
   // MACGalleryDelPhotoSourceType sourceType = MACGalleryDelPhotoSourceTypeLocal;
    MACGalleryPhoto *photo;
//    NSString *thumbPath;
//    NSString *fullsizePath;
    NSDictionary* assetDic;
    assetDic =  [assetArray objectAtIndex:index] ;
    photo = [[MACGalleryPhoto alloc]initWithAssetDictionary:assetDic delegate:self];
//    
//    if( sourceType == MACGalleryDelPhotoSourceTypeLocal )
//    {
//        if (isAssetDic) {
//            assetDic = [_photoSource photoGallery:self assetDictionaryAtIndex:index];
//            photo = [[MACGalleryPhoto alloc]initWithAssetDictionary:assetDic delegate:self];
//        }else{
//            thumbPath = [_photoSource photoGallery:self filePathForPhotoSize:MACGalleryDelPhotoSizeThumbnail atIndex:index];
//            fullsizePath = [_photoSource photoGallery:self filePathForPhotoSize:MACGalleryDelPhotoSizeFullsize atIndex:index];
//            photo = [[MACGalleryPhoto alloc] initWithThumbnailPath:thumbPath fullsizePath:fullsizePath delegate:self];
//        }
//    }
//    else if( sourceType == MACGalleryDelPhotoSourceTypeNetwork )
//    {
//        thumbPath = [_photoSource photoGallery:self urlForPhotoSize:MACGalleryDelPhotoSizeThumbnail atIndex:index];
//        fullsizePath = [_photoSource photoGallery:self urlForPhotoSize:MACGalleryDelPhotoSizeFullsize atIndex:index];
 ...
 *	to allow the tab bar to rotate only when a MACGalleryDelController is the visible controller. Sweet.
 */

@implementation UINavigationController (MACGalleryDel)

- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
    if([self.visibleViewController isKindOfClass:[MACGalleryDelViewController class]])
    {
        return YES;
    }
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    if([self.visibleViewController isKindOfClass:[MACGalleryDelViewController class]])
    {
        MACGalleryDelViewController *galleryController = (MACGalleryDelViewController*)self.visibleViewController;
...
@implementation UITabBarController (MACGalleryDel)
...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
    ...
    if( [self.selectedViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        // see if the current controller in the stack is a gallery
        if([navController.visibleViewController isKindOfClass:[MACGalleryDelViewController class]])
        {
            return YES;
        }
    }
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    if([self.selectedViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        if([navController.visibleViewController isKindOfClass:[MACGalleryDelViewController class]])
        {
            MACGalleryDelViewController *galleryController = (MACGalleryDelViewController*)navController.visibleViewController;
            [galleryController resetImageViewZoomLevels];
        }
    }
}
