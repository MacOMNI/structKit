...
@interface MACGalleryPhoto (Private)
...
@implementation MACGalleryPhoto{
...
- (void)loadFullsizeInThread
{
   @autoreleasepool{
    ...
    if (isAssetDic) {
        ...
        _fullsize = [UIImage imageWithCGImage:[[asset defaultRepresentation] fullScreenImage]];
...
- (void)loadThumbnailInThread
{
    ...
    if (isAssetDic) {
        ALAsset* asset = [assetDictionary objectForKey:@"asset"];
        _thumbnail = [UIImage imageWithCGImage:[[asset defaultRepresentation] fullScreenImage]];
