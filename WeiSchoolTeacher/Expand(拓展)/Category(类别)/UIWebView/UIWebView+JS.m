 ...
 *  @return 网页颜色
 */
- (NSString *)webColorString:(UIColor *)color
{
    CGFloat *arrRGBA = [self getRGB:color];
    int r = arrRGBA[0] * 255;
    int g = arrRGBA[1] * 255;
    int b = arrRGBA[2] * 255;
    NSLog(@"%d,%d,%d", r, g, b);
...
{
    NSString * jsString = [NSString stringWithFormat:@"document.body.style.backgroundColor = '%@'",[self webColorString:color]];
...
- (void)setFontColor:(UIColor *)color withTag:(NSString *)tagName
{
    NSString *jsString = [NSString stringWithFormat:
                          ...
                          nodes[i].style.color = '%@';}", tagName, [self webColorString:color]];
    [self stringByEvaluatingJavaScriptFromString:jsString];
}
/// 改变指定标签的字体大小
